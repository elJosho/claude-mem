...
`+JSON.stringify(o,null,2):_=" "+this.formatData(o));let l="";if(n){let{sessionId:T,memorySessionId:S,correlationId:h,...p}=n;Object.keys(p).length>0&&(l=` {${Object.entries(p).map(([g,f])=>`${g}=${f}`).join(", ")}}`)}let E=`[${i}] [${a}] [${d}] ${u}${r}${l}${_}`;if(this.logFilePath)try{(0,D.appendFileSync)(this.logFilePath,E+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),_=u?`${u[1].split("/").pop()}:${u[2]}`:"unknown",l={...r,location:_};return this.warn(e,`[HAPPY-PATH] ${t}`,l,n),o}},m=new ne;var Gt={};function jt(){return typeof __dirname<"u"?__dirname:(0,b.dirname)((0,Ne.fileURLToPath)(Gt.url))}var Ht=jt();function Xt(){if(process.env.CLAUDE_MEM_DATA_DIR)return process.env.CLAUDE_MEM_DATA_DIR;let s=(0,b.join)((0,oe.homedir)(),".claude-mem"),e=(0,b.join)(s,"settings.json");try{if((0,G.existsSync)(e)){let{readFileSync:t}=require("fs"),r=JSON.parse(t(e,"utf-8")),n=r.env??r;if(n.CLAUDE_MEM_DATA_DIR)return n.CLAUDE_MEM_DATA_DIR}}catch{}return s}var N=Xt(),v=process.env.CLAUDE_CONFIG_DIR||(0,b.join)((0,oe.homedir)(),".claude"),_s=(0,b.join)(v,"plugins","marketplaces","thedotmack"),ps=(0,b.join)(N,"archives"),ls=(0,b.join)(N,"logs"),Es=(0,b.join)(N,"trash"),gs=(0,b.join)(N,"backups"),Ts=(0,b.join)(N,"modes"),fs=(0,b.join)(N,"settings.json"),Ce=(0,b.join)(N,"claude-mem.db"),Ss=(0,b.join)(N,"vector-db"),Ie=(0,b.join)(N,"observer-sessions"),C="observer-sessions";function B(s){return s===C?"unknown-project":s}var bs=(0,b.join)(v,"settings.json"),hs=(0,b.join)(v,"commands"),Os=(0,b.join)(v,"CLAUDE.md");function Le(s){(0,G.mkdirSync)(s,{recursive:!0})}function De(){return(0,b.join)(Ht,"..")}var ie=/<system-reminder>[\s\S]*?<\/system-reminder>/g,ve=100;function Bt(s){let e=(s.match(/<private>/g)||[]).length,t=(s.match(/<claude-mem-context>/g)||[]).length,r=(s.match(/<system_instruction>/g)||[]).length,n=(s.match(/<system-instruction>/g)||[]).length,o=(s.match(/<persisted-output>/g)||[]).length,i=(s.match(/<system-reminder>/g)||[]).length,a=(s.match(/<skill_content[\s>]/g)||[]).length,d=(s.match(/<observed_from_primary_session>/g)||[]).length;return e+t+r+n+o+i+a+d}function Wt(s){let e=Bt(s);return e>ve&&m.warn("SYSTEM","tag count exceeds limit",void 0,{tagCount:e,maxAllowed:ve,contentLength:s.length}),s.replace(/<claude-mem-context>[\s\S]*?<\/claude-mem-context>/g,"").replace(/<private>[\s\S]*?<\/private>/g,"").replace(/<system_instruction>[\s\S]*?<\/system_instruction>/g,"").replace(/<system-instruction>[\s\S]*?<\/system-instruction>/g,"").replace(/<persisted-output>[\s\S]*?<\/persisted-output>/g,"").replace(ie,"").replace(/<skill_content[\s\S]*?<\/skill_content>/g,"").replace(/<observed_from_primary_session>[\s\S]*?<\/observed_from_primary_session>/g,"").trim()}function I(s){return Wt(s)}function W(s){if(s===null)return null;let e=I(s);return e===""?null:e}function x(s){return{...s,type:s.type,title:W(s.title),subtitle:W(s.subtitle),narrative:W(s.narrative),facts:s.facts.map(e=>I(e)).filter(e=>e.length>0),concepts:s.concepts.map(e=>I(e)).filter(e=>e.length>0),files_read:s.files_read.map(e=>I(e)),files_modified:s.files_modified.map(e=>I(e))}}function Y(s){return{...s,request:I(s.request),investigated:I(s.investigated),learned:I(s.learned),completed:I(s.completed),next_steps:I(s.next_steps),notes:W(s.notes)}}var ye=require("crypto");var Yt=3e4;function V(s,e,t){return(0,ye.createHash)("sha256").update([s||"",e||"",t||""].join("\0")).digest("hex").slice(0,16)}function q(s,e,t){let r=t-Yt;return s.prepare("SELECT id, created_at_epoch FROM observations WHERE content_hash = ? AND created_at_epoch > ?").get(e,r)}function ae(s){if(!s)return[];try{let e=JSON.parse(s);return Array.isArray(e)?e:[String(e)]}catch{return[s]}}var O="claude";function Vt(s){return s.trim().toLowerCase().replace(/\s+/g,"-")}function y(s){if(!s)return O;let e=Vt(s);return e?e==="transcript"||e.includes("codex")?"codex":e.includes("cursor")?"cursor":e.includes("opencode")?"opencode":e.includes("claude")?"claude":e:O}function Me(s){let e=["claude","opencode","codex","cursor"];return[...s].sort((t,r)=>{let n=e.indexOf(t),o=e.indexOf(r);return n!==-1||o!==-1?n===-1?1:o===-1?-1:n-o:t.localeCompare(r)})}function qt(s,e){return{customTitle:s,platformSource:e?y(e):void 0}}function Ue(s){return s.replace(/\r\n/g,`
`).replace(/\r/g,`
`).trim()}var K=class{db;constructor(e=Ce){e!==":memory:"&&Le(N),this.db=new xe.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.addObservationContentHashColumn(),this.addSessionCustomTitleColumn(),this.addSessionPlatformSourceColumn(),this.addObservationModelColumns()}initializeSchema(){this.db.run(`
    ...
    `,n=[C];return t&&(r+=" AND COALESCE(platform_source, ?) = ?",n.push(O,t)),r+=" ORDER BY project ASC",this.db.prepare(r).all(...n).map(i=>i.project)}getProjectCatalog(){let e=this.db.prepare(`
      SELECT
    ...
    `).get(e)||null}getFilesForSession(e){let r=this.db.prepare(`
      ...
      FROM observations
      WHERE memory_session_id = ?
    `).all(e),n=new Set,o=new Set;for(let i of r)ae(i.files_read).forEach(a=>n.add(a)),ae(i.files_modified).forEach(a=>o.add(a));return{filesRead:Array.from(n),filesModified:Array.from(o)}}getSessionById(e){return this.db.prepare(`
    ...
    `).get(e).count}createSDKSession(e,t,r,n,o){let i=new Date,a=i.getTime(),d=qt(n,o),u=d.platformSource??O,_=this.db.prepare(`
    ...
    `).get(e);if(_){let T=B(t);if(T&&this.db.prepare(`
      ...
      VALUES (?, NULL, ?, ?, ?, ?, ?, ?, 'active')
    `).run(e,l,u,r,d.customTitle||null,i.toISOString(),a),this.db.prepare("SELECT id FROM sdk_sessions WHERE content_session_id = ?").get(e).id}lookupSessionDbId(e){return this.db.prepare("SELECT id FROM sdk_sessions WHERE content_session_id = ?").get(e)?.id??null}pruneInternalRecords(){let t=this.db.prepare(`
    ...
    `).run(C).changes,d=this.db.prepare(`
      ...
      WHERE project = ? OR project IS NULL OR project = ''
    `).run(C).changes;return(d>0||i>0||t>0||n>0)&&m.info("DB",`Startup pruning: removed ${d} internal sessions, ${i} orphaned prompts, ${t} observations, ${n} summaries (observer/empty project)`),{sessions:d,prompts:i,observations:t,summaries:n}}saveUserPrompt(e,t,r){let n=new Date,o=n.getTime();return this.db.prepare(`
      INSERT INTO user_prompts
      (content_session_id, prompt_number, prompt_text, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?)
    `).run(e,t,r,n.toISOString(),o).lastInsertRowid}saveNextUserPromptAtomic(e,t){let n=Ue(t),o=n.trimStart();return o.startsWith("You are a Claude-Mem")||o.startsWith("Hello memory agent")||o.startsWith("<observed_from_primary_session>")||o.includes("specialized observer tool for creating searchable memory")?(m.debug("DEDUP","Rejected observer-session content in saveNextUserPromptAtomic",{contentSessionId:e,preview:o.substring(0,60)}),{id:-1,promptNumber:0,created_at_epoch:Date.now(),duplicateSkipped:!0}):this.db.transaction(()=>{let a=this.getLatestUserPrompt(e);if(a&&Ue(a.prompt_text)===n&&Date.now()-a.created_at_epoch<8e3)return m.debug("DEDUP","Skipped duplicate user_prompt (same text within window)",{contentSessionId:e,promptNumber:a.prompt_number}),{id:a.id,promptNumber:a.prompt_number,created_at_epoch:a.created_at_epoch,duplicateSkipped:!0};let u=this.db.prepare(`
        SELECT COUNT(*) as c FROM user_prompts WHERE content_session_id = ?
      `).get(e).c+1,_=new Date,l=_.getTime(),T=this.db.prepare(`
        INSERT INTO user_prompts
        (content_session_id, prompt_number, prompt_text, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?)
      `).run(e,u,n,_.toISOString(),l);return{id:Number(T.lastInsertRowid),promptNumber:u,created_at_epoch:l,duplicateSkipped:!1}})()}getUserPrompt(e,t){return this.db.prepare(`
      SELECT prompt_text
      FROM user_prompts
      WHERE content_session_id = ? AND prompt_number = ?
      LIMIT 1
    `).get(e,t)?.prompt_text??null}storeObservation(e,t,r,n,o=0,i,a){let d=x(r),u=i??Date.now(),_=new Date(u).toISOString(),l=V(e,d.title,d.narrative),E=q(this.db,l,u);if(E)return{id:E.id,createdAtEpoch:E.created_at_epoch};let S=this.db.prepare(`
      INSERT INTO observations
      (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
       files_read, files_modified, prompt_number, discovery_tokens, content_hash, created_at, created_at_epoch,
       generated_by_model)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,d.type,d.title,d.subtitle,JSON.stringify(d.facts),d.narrative,JSON.stringify(d.concepts),JSON.stringify(d.files_read),JSON.stringify(d.files_modified),n||null,o,l,_,u,a||null);return{id:Number(S.lastInsertRowid),createdAtEpoch:u}}storeSummary(e,t,r,n,o=0,i){let a=Y(r),d=i??Date.now(),u=new Date(d).toISOString(),l=this.db.prepare(`
      INSERT INTO session_summaries
      (memory_session_id, project, request, investigated, learned, completed,
       next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,a.request,a.investigated,a.learned,a.completed,a.next_steps,a.notes,n||null,o,u,d);return{id:Number(l.lastInsertRowid),createdAtEpoch:d}}storeObservations(e,t,r,n,o,i=0,a,d){let u=a??Date.now(),_=new Date(u).toISOString();return this.db.transaction(()=>{let E=[],T=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, content_hash, created_at, created_at_epoch,
         generated_by_model)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let h of r){let p=x(h),A=V(e,p.title,p.narrative),g=q(this.db,A,u);if(g){E.push(g.id);continue}let f=T.run(e,t,p.type,p.title,p.subtitle,JSON.stringify(p.facts),p.narrative,JSON.stringify(p.concepts),JSON.stringify(p.files_read),JSON.stringify(p.files_modified),o||null,i,A,_,u,d||null);E.push(Number(f.lastInsertRowid))}let S=null;if(n){let h=Y(n),A=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,h.request,h.investigated,h.learned,h.completed,h.next_steps,h.notes,o||null,i,_,u);S=Number(A.lastInsertRowid)}return{observationIds:E,summaryId:S,createdAtEpoch:u}})()}storeObservationsAndMarkComplete(e,t,r,n,o,i,a,d=0,u,_){let l=u??Date.now(),E=new Date(l).toISOString();return this.db.transaction(()=>{let S=[],h=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, content_hash, created_at, created_at_epoch,
         generated_by_model)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let g of r){let f=x(g),H=V(e,f.title,f.narrative),he=q(this.db,H,l);if(he){S.push(he.id);continue}let Mt=h.run(e,t,f.type,f.title,f.subtitle,JSON.stringify(f.facts),f.narrative,JSON.stringify(f.concepts),JSON.stringify(f.files_read),JSON.stringify(f.files_modified),a||null,d,H,E,l,_||null);S.push(Number(Mt.lastInsertRowid))}let p;if(n){let g=Y(n),H=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      ...
      `).run(l,o),{observationIds:S,summaryId:p,createdAtEpoch:l}})()}getSessionSummariesByIds(e,t={}){if(e.length===0)return[];let{orderBy:r="date_desc",limit:n,project:o}=t,i=r==="date_asc"?"ASC":"DESC",a=n?`LIMIT ${n}`:"",d=e.map(()=>"?").join(","),u=[...e],_=o?`WHERE id IN (${d}) AND project = ?`:`WHERE id IN (${d})`;return o&&u.push(o),this.db.prepare(`
      SELECT * FROM session_summaries
      ${_}
      ORDER BY created_at_epoch ${i}
      ${a}
    `).all(...u)}getUserPromptsByIds(e,t={}){if(e.length===0)return[];let{orderBy:r="date_desc",limit:n,project:o}=t,i=r==="date_asc"?"ASC":"DESC",a=n?`LIMIT ${n}`:"",d=e.map(()=>"?").join(","),u=[...e],_=o?"AND s.project = ?":"";return o&&u.push(o),this.db.prepare(`
      SELECT
        up.*,
        s.project,
        s.memory_session_id
      ...
      ${a}
    `).all(...u)}getTimelineAroundTimestamp(e,t=10,r=10,n){return this.getTimelineAroundObservation(null,e,t,r,n)}getTimelineAroundObservation(e,t,r=10,n=10,o){let i=o?"AND project = ?":"",a=o?[o]:[],d,u;if(e!==null){let p=`
      ...
      `,A=`
        ...
        ORDER BY id ASC
        LIMIT ?
      `;try{let g=this.db.prepare(p).all(e,...a,r+1),f=this.db.prepare(A).all(e,...a,n+1);if(g.length===0&&f.length===0)return{observations:[],sessions:[],prompts:[]};d=g.length>0?g[g.length-1].created_at_epoch:t,u=f.length>0?f[f.length-1].created_at_epoch:t}catch(g){return m.error("DB","Error getting boundary observations",void 0,{error:g,project:o}),{observations:[],sessions:[],prompts:[]}}}else{let p=`
      ...
      `,A=`
        ...
        ORDER BY created_at_epoch ASC
        LIMIT ?
      `;try{let g=this.db.prepare(p).all(t,...a,r),f=this.db.prepare(A).all(t,...a,n+1);if(g.length===0&&f.length===0)return{observations:[],sessions:[],prompts:[]};d=g.length>0?g[g.length-1].created_at_epoch:t,u=f.length>0?f[f.length-1].created_at_epoch:t}catch(g){return m.error("DB","Error getting boundary timestamps",void 0,{error:g,project:o}),{observations:[],sessions:[],prompts:[]}}}let _=`
      SELECT *
      FROM observations
    ...
    `).all(...e)}getSessionSummaryById(e){return this.db.prepare(`
      ...
      WHERE id = ?
      LIMIT 1
    `).get(e)||null}getOrCreateManualSession(e){let t=B(e),r=`manual-${t}`,n=`manual-content-${t}`;if(this.db.prepare("SELECT memory_session_id FROM sdk_sessions WHERE memory_session_id = ?").get(r))return r;let i=new Date;return this.db.prepare(`
    ...
    ORDER BY o.created_at_epoch DESC
    LIMIT ?
  `).all(e,...n,...i,...r?[r]:[],t.totalObservationCount)}function le(s,e,t,r){return s.db.prepare(`
    SELECT
      ss.id,
    ...
    ORDER BY ss.created_at_epoch DESC
    LIMIT ?
  `).all(e,...r?[r]:[],t.sessionCount+ue)}function He(s,e,t,r){let n=Array.from(t.observationTypes),o=n.map(()=>"?").join(","),i=Array.from(t.observationConcepts),a=i.map(()=>"?").join(","),d=e.map(()=>"?").join(",");return s.db.prepare(`
    SELECT
      o.id,
      o.memory_session_id,
      COALESCE(s.platform_source, 'claude') as platform_source,
    ...
    ORDER BY o.created_at_epoch DESC
    LIMIT ?
  `).all(...e,...n,...i,...r?[r]:[],t.totalObservationCount)}function Xe(s,e,t,r){let n=e.map(()=>"?").join(",");return s.db.prepare(`
    SELECT
      ss.id,
      ss.memory_session_id,
      COALESCE(s.platform_source, 'claude') as platform_source,
    ...
    ORDER BY ss.created_at_epoch DESC
    LIMIT ?
  `).all(...e,...r?[r]:[],t.sessionCount+ue)}function zt(s){return s.replace(/\//g,"-")}function Qt(s){try{if(!(0,Z.existsSync)(s))return{userMessage:"",assistantMessage:""};let e=(0,Z.readFileSync)(s,"utf-8").trim();if(!e)return{userMessage:"",assistantMessage:""};let t=e.split(`
`).filter(n=>n.trim()),r="";for(let n=t.length-1;n>=0;n--)try{let o=t[n];if(!o.includes('"type":"assistant"'))continue;let i=JSON.parse(o);if(i.type==="assistant"&&i.message?.content&&Array.isArray(i.message.content)){let a="";for(let d of i.message.content)d.type==="text"&&(a+=d.text);if(a=a.replace(ie,"").trim(),a){r=a;break}}}catch(o){m.debug("PARSER","Skipping malformed transcript line",{lineIndex:n},o);continue}return{userMessage:"",assistantMessage:r}}catch(e){return m.failure("WORKER","Failed to extract prior messages from transcript",{transcriptPath:s},e),{userMessage:"",assistantMessage:""}}}function Ee(s,e,t,r){if(!e.showLastMessage||s.length===0)return{userMessage:"",assistantMessage:""};let n=s.find(d=>d.memory_session_id!==t);if(!n)return{userMessage:"",assistantMessage:""};let o=n.memory_session_id,i=zt(r),a=je.default.join(v,"projects",i,`${o}.jsonl`);return Qt(a)}function Ge(s,e){let t=e[0]?.id;return s.map((r,n)=>{let o=n===0?null:e[n+1];return{...r,displayEpoch:o?o.created_at_epoch:r.created_at_epoch,displayTime:o?o.created_at:r.created_at,shouldShowLink:r.id!==t}})}function ge(s,e){let t=[...s.map(r=>({type:"observation",data:r})),...e.map(r=>({type:"summary",data:r}))];return t.sort((r,n)=>{let o=r.type==="observation"?r.data.created_at_epoch:r.data.displayEpoch,i=n.type==="observation"?n.data.created_at_epoch:n.data.displayEpoch;return o-i}),t}function Be(s,e){return new Set(s.slice(0,e).map(t=>t.id))}function We(){let s=new Date,e=s.toLocaleDateString("en-CA"),t=s.toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit",hour12:!0}).toLowerCase().replace(" ",""),r=s.toLocaleTimeString("en-US",{timeZoneName:"short"}).split(" ").pop();return`${e} ${t} ${r}`}function Ye(s){return[`# $CMEM ${s} ${We()}`,""]}function Ve(){return[`Legend: \u{1F3AF}session ${R.getInstance().getActiveMode().observation_types.map(t=>`${t.emoji}${t.id}`).join(" ")}`,"Format: ID TIME TYPE TITLE","Fetch details: get_observations([IDs]) | Search: mem-search skill",""]}function qe(){return[]}function Ke(){return[]}function Je(s,e){let t=[],r=[`${s.totalObservations} obs (${s.totalReadTokens.toLocaleString()}t read)`,`${s.totalDiscoveryTokens.toLocaleString()}t work`];return s.totalDiscoveryTokens>0&&(e.showSavingsAmount||e.showSavingsPercent)&&(e.showSavingsPercent?r.push(`${s.savingsPercent}% savings`):e.showSavingsAmount&&r.push(`${s.savings.toLocaleString()}t saved`)),t.push(`Stats: ${r.join(" | ")}`),t.push(""),t}function ze(s){return[`### ${s}`]}function Qe(s){return s.toLowerCase().replace(" am","a").replace(" pm","p")}function Ze(s,e,t){let r=s.title||"Untitled",n=R.getInstance().getTypeIcon(s.type),o=e?Qe(e):'"';return`${s.id} ${o} ${n} ${r}`}function et(s,e,t,r){let n=[],o=s.title||"Untitled",i=R.getInstance().getTypeIcon(s.type),a=e?Qe(e):'"',{readTokens:d,discoveryDisplay:u}=F(s,r);n.push(`**${s.id}** ${a} ${i} **${o}**`),t&&n.push(t);let _=[];return r.showReadTokens&&_.push(`~${d}t`),r.showWorkTokens&&_.push(u),_.length>0&&n.push(_.join(" ")),n.push(""),n}function tt(s,e){return[`S${s.id} ${s.request||"Session started"} (${e})`]}function P(s,e){return e?[`**${s}**: ${e}`,""]:[]}function st(s){return s.assistantMessage?["","---","","**Previously**","",`A: ${s.assistantMessage}`,""]:[]}function rt(s,e){return["",`Access ${Math.round(s/1e3)}k tokens of past work via get_observations([IDs]) or mem-search skill.`]}function nt(s){return`# $CMEM ${s} ${We()}

No previous sessions found.`}function ot(){let s=new Date,e=s.toLocaleDateString("en-CA"),t=s.toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit",hour12:!0}).toLowerCase().replace(" ",""),r=s.toLocaleTimeString("en-US",{timeZoneName:"short"}).split(" ").pop();return`${e} ${t} ${r}`}function it(s){return["",`${c.bright}${c.cyan}[${s}] recent context, ${ot()}${c.reset}`,`${c.gray}${"\u2500".repeat(60)}${c.reset}`,""]}function at(){let e=R.getInstance().getActiveMode().observation_types.map(t=>`${t.emoji} ${t.id}`).join(" | ");return[`${c.dim}Legend: session-request | ${e}${c.reset}`,""]}function dt(){return[`${c.bright}Column Key${c.reset}`,`${c.dim}  Read: Tokens to read this observation (cost to learn it now)${c.reset}`,`${c.dim}  Work: Tokens spent on work that produced this record ( research, building, deciding)${c.reset}`,""]}function ct(){return[`${c.dim}Context Index: This semantic index (titles, types, files, tokens) is usually sufficient to understand past work.${c.reset}`,"",`${c.dim}When you need implementation details, rationale, or debugging context:${c.reset}`,`${c.dim}  - Fetch by ID: get_observations([IDs]) for observations visible in this index${c.reset}`,`${c.dim}  - Search history: Use the mem-search skill for past decisions, bugs, and deeper research${c.reset}`,`${c.dim}  - Trust this index over re-reading code for past decisions and learnings${c.reset}`,""]}function ut(s,e){let t=[];if(t.push(`${c.bright}${c.cyan}Context Economics${c.reset}`),t.push(`${c.dim}  Loading: ${s.totalObservations} observations (${s.totalReadTokens.toLocaleString()} tokens to read)${c.reset}`),t.push(`${c.dim}  Work investment: ${s.totalDiscoveryTokens.toLocaleString()} tokens spent on research, building, and decisions${c.reset}`),s.totalDiscoveryTokens>0&&(e.showSavingsAmount||e.showSavingsPercent)){let r="  Your savings: ";e.showSavingsAmount&&e.showSavingsPercent?r+=`${s.savings.toLocaleString()} tokens (${s.savingsPercent}% reduction from reuse)`:e.showSavingsAmount?r+=`${s.savings.toLocaleString()} tokens`:r+=`${s.savingsPercent}% reduction from reuse`,t.push(`${c.green}${r}${c.reset}`)}return t.push(""),t}function mt(s){return[`${c.bright}${c.cyan}${s}${c.reset}`,""]}function _t(s){return[`${c.dim}${s}${c.reset}`]}function pt(s,e,t,r){let n=s.title||"Untitled",o=R.getInstance().getTypeIcon(s.type),{readTokens:i,discoveryTokens:a,workEmoji:d}=F(s,r),u=t?`${c.dim}${e}${c.reset}`:" ".repeat(e.length),_=r.showReadTokens&&i>0?`${c.dim}(~${i}t)${c.reset}`:"",l=r.showWorkTokens&&a>0?`${c.dim}(${d} ${a.toLocaleString()}t)${c.reset}`:"";return`  ${c.dim}#${s.id}${c.reset}  ${u}  ${o}  ${n} ${_} ${l}`}function lt(s,e,t,r,n){let o=[],i=s.title||"Untitled",a=R.getInstance().getTypeIcon(s.type),{readTokens:d,discoveryTokens:u,workEmoji:_}=F(s,n),l=t?`${c.dim}${e}${c.reset}`:" ".repeat(e.length),E=n.showReadTokens&&d>0?`${c.dim}(~${d}t)${c.reset}`:"",T=n.showWorkTokens&&u>0?`${c.dim}(${_} ${u.toLocaleString()}t)${c.reset}`:"";return o.push(`  ${c.dim}#${s.id}${c.reset}  ${l}  ${a}  ${c.bright}${i}${c.reset}`),r&&o.push(`    ${c.dim}${r}${c.reset}`),(E||T)&&o.push(`    ${E} ${T}`),o.push(""),o}function Et(s,e){let t=`${s.request||"Session started"} (${e})`;return[`${c.yellow}#S${s.id}${c.reset} ${t}`,""]}function j(s,e,t){return e?[`${t}${s}:${c.reset} ${e}`,""]:[]}function gt(s){return s.assistantMessage?["","---","",`${c.bright}${c.magenta}Previously${c.reset}`,"",`${c.dim}A: ${s.assistantMessage}${c.reset}`,""]:[]}function Tt(s,e){let t=Math.round(s/1e3);return["",`${c.dim}Access ${t}k tokens of past research & decisions for just ${e.toLocaleString()}t. Use the claude-mem skill to access memories by ID.${c.reset}`]}function ft(s){return`
${c.bright}${c.cyan}[${s}] recent context, ${ot()}${c.reset}
${c.gray}${"\u2500".repeat(60)}${c.reset}

${c.dim}No previous sessions found for this project yet.${c.reset}
`}function St(s,e,t,r){let n=[];return r?n.push(...it(s)):n.push(...Ye(s)),r?n.push(...at()):n.push(...Ve()),r?n.push(...dt()):n.push(...qe()),r?n.push(...ct()):n.push(...Ke()),Q(t)&&(r?n.push(...ut(e,t)):n.push(...Je(e,t))),n}var Te=M(require("path"),1);function se(s){if(!s)return[];try{let e=JSON.parse(s);return Array.isArray(e)?e:[]}catch(e){return m.debug("PARSER","Failed to parse JSON array, using empty fallback",{preview:s?.substring(0,50)},e),[]}}function fe(s){return new Date(s).toLocaleString("en-US",{month:"short",day:"numeric",hour:"numeric",minute:"2-digit",hour12:!0})}function Se(s){return new Date(s).toLocaleString("en-US",{hour:"numeric",minute:"2-digit",hour12:!0})}function ht(s){return new Date(s).toLocaleString("en-US",{month:"short",day:"numeric",year:"numeric"})}function bt(s,e){return Te.default.isAbsolute(s)?Te.default.relative(e,s):s}function Ot(s,e,t){let r=se(s);if(r.length>0)return bt(r[0],e);if(t){let n=se(t);if(n.length>0)return bt(n[0],e)}return"General"}function Zt(s){let e=new Map;for(let r of s){let n=r.type==="observation"?r.data.created_at:r.data.displayTime,o=ht(n);e.has(o)||e.set(o,[]),e.get(o).push(r)}let t=Array.from(e.entries()).sort((r,n)=>{let o=new Date(r[0]).getTime(),i=new Date(n[0]).getTime();return o-i});return new Map(t)}function At(s,e){return e.fullObservationField==="narrative"?s.narrative:s.facts?se(s.facts).join(`
`):null}function es(s,e,t,r){let n=[];n.push(...ze(s));let o="";for(let i of e)if(i.type==="summary"){let a=i.data,d=fe(a.displayTime);n.push(...tt(a,d))}else{let a=i.data,d=Se(a.created_at),_=d!==o?d:"";if(o=d,t.has(a.id)){let E=At(a,r);n.push(...et(a,_,E,r))}else n.push(Ze(a,_,r))}return n}function ts(s,e,t,r,n){let o=[];o.push(...mt(s));let i=null,a="";for(let d of e)if(d.type==="summary"){i=null,a="";let u=d.data,_=fe(u.displayTime);o.push(...Et(u,_))}else{let u=d.data,_=Ot(u.files_modified,n,u.files_read),l=Se(u.created_at),E=l!==a;a=l;let T=t.has(u.id);if(_!==i&&(o.push(..._t(_)),i=_),T){let S=At(u,r);o.push(...lt(u,l,E,S,r))}else o.push(pt(u,l,E,r))}return o.push(""),o}function ss(s,e,t,r,n,o){return o?ts(s,e,t,r,n):es(s,e,t,r)}function Rt(s,e,t,r,n){let o=[],i=Zt(s);for(let[a,d]of i)o.push(...ss(a,d,e,t,r,n));return o}function Nt(s,e,t){return!(!s.showLastSummary||!e||!!!(e.investigated||e.learned||e.completed||e.next_steps)||t&&e.created_at_epoch<=t.created_at_epoch)}function Ct(s,e){let t=[];return e?(t.push(...j("Investigated",s.investigated,c.blue)),t.push(...j("Learned",s.learned,c.yellow)),t.push(...j("Completed",s.completed,c.green)),t.push(...j("Next Steps",s.next_steps,c.magenta))):(t.push(...P("Investigated",s.investigated)),t.push(...P("Learned",s.learned)),t.push(...P("Completed",s.completed)),t.push(...P("Next Steps",s.next_steps))),t}function It(s,e){return e?gt(s):st(s)}function Lt(s,e,t){return!Q(e)||s.totalDiscoveryTokens<=0||s.savings<=0?[]:t?Tt(s.totalDiscoveryTokens,s.totalReadTokens):rt(s.totalDiscoveryTokens,s.totalReadTokens)}var rs=Dt.default.join((0,vt.homedir)(),".claude","plugins","marketplaces","thedotmack","plugin",".install-version");function ns(){try{return new K}catch(s){if(s.code==="ERR_DLOPEN_FAILED"){try{(0,yt.unlinkSync)(rs)}catch(e){m.debug("SYSTEM","Marker file cleanup failed (may not exist)",{},e)}return m.error("SYSTEM","Native module rebuild needed - restart Claude Code to auto-fix"),null}throw s}}function os(s,e){return e?ft(s):nt(s)}function is(s,e,t,r,n,o,i){let a=[],d=_e(e);a.push(...St(s,d,r,i));let u=t.slice(0,r.sessionCount),_=Ge(u,t),l=ge(e,_),E=Be(e,r.fullObservationCount);a.push(...Rt(l,E,r,n,i));let T=t[0],S=e[0];Nt(r,T,S)&&a.push(...Ct(T,i));let h=Ee(e,r,o,n);return a.push(...It(h,i)),a.push(...Lt(d,r,i)),a.join(`
`).trimEnd()}async function be(s,e=!1){let t=ce(),r=s?.cwd??process.cwd(),n=$e(r),o=s?.platform_source,i=s?.projects||[n];s?.full&&(t.totalObservationCount=999999,t.sessionCount=999999);let a=ns();if(!a)return"";try{let d=i.length>1?He(a,i,t,o):pe(a,n,t,o),u=i.length>1?Xe(a,i,t,o):le(a,n,t,o);return d.length===0&&u.length===0?os(n,e):is(n,d,u,t,r,s?.session_id,e)}finally{a.close()}}0&&(module.exports={generateContext});
